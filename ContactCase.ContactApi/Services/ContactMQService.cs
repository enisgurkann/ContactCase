...
{
    public class ContactMQService : BackgroundService
    {
        private IConnection _connection;
        private IModel _channel;
        private readonly IServiceScopeFactory _serviceScopeFactory;

        public ContactMQService(IServiceScopeFactory serviceScopeFactory)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (ch, ea) => {
                ...
                if (content.Contains("RAPOROLUSTUR")) {
                    Guid UUID = Guid.Parse(content.Split('$')[1]);
                    string TAG = content.Split('$')[2];

                    using (var scope = _serviceScopeFactory.CreateScope()) {
                        var dataContext = scope.ServiceProvider.GetService<AppDBContext>();
                        var count = await dataContext.ContactInfos.CountAsync(x => x.InfoType == TAG);

                        var message = $"RAPOROLUSTURULDU${UUID}${count}";
